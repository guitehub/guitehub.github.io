\# Cuisine — cahier des charges



Outil du site « Le Hub de la Guite » (repo `guitehub/guitehub.github.io`), servi à `/outils/cuisine/`.
...
\- Si un point n'est pas couvert, choisis l'option la plus simple, note-la dans `outils/cuisine/DECISIONS.md` et continue. Ne t'arrête pour me demander que si l'action est destructive ou touche au blog existant.
...
| Hébergement | Repo `guitehub.github.io`, dossier `/outils/cuisine/` | Validé |
...
| Stockage des recettes | Un fichier JSON par recette dans `\_data/cuisine/recettes/` | Par défaut |
...
| Section « Outils » | La page `pages/outils.md` existe déjà (catégorie `tool`). Cuisine y apparaît via son article, comme toute app (convention §3). Aucun fichier existant ne bouge (aucune URL cassée) | Validé |
...
\- \*\*Navigation.\*\* `\_includes/nav.html` liste les pages marquées `is\_main: true`, triées par `title`.
...
\- \*\*Convention projets / outils\*\* (détaillée dans `README.md`). Deux axes indépendants :

&#x20; - \*\*Nature = dossier = catégorie = page.\*\* `projets/` ↔ catégorie `project` ↔ page Projets ; `outils/` ↔ catégorie `tool` ↔ page Outils.

&#x20; - \*\*Forme.\*\* Soit un fichier unique `outils/nom.html` (URL `/outils/nom.html`), soit un dossier `outils/nom/` avec un `index.html` (URL `/outils/nom/`).

&#x20; - \*\*Référencement.\*\* Une app apparaît sur sa page uniquement via son article `\_posts/AAAA-MM-JJ-nom.md`, dans la bonne catégorie. Pas de liste parallèle.

&#x20; - Cuisine est un \*\*outil\*\* sous la \*\*forme dossier\*\* : `outils/cuisine/index.html`, article en catégorie `tool`.

\- \*\*Apps existantes.\*\* Toutes sous la forme fichier unique, sans dépendance, avec une page autonome hors layout :

&#x20; - `outils/icomkr.html` (outil, catégorie `tool`) ;

&#x20; - `projets/flechettes.html` et `projets/yams.html` (projets, catégorie `project`).

&#x20; - Leur `localStorage` utilise un préfixe et une sonde pour le mode privé iOS.

&#x20; - Chacune a un article dans `\_posts/` (`layout: post`, catégorie `tool` ou `project`, `image:` vignette).
...
\_specs/cuisine.md                  # ce document (dossier \_ : non publié)
...
pages/outils.md                    # EXISTE DÉJÀ : page « Outils » (catégorie tool), ne pas modifier

\_posts/AAAA-MM-JJ-cuisine.md       # article de présentation

scripts/cuisine-check.mjs          # validation des données

.github/workflows/cuisine-check.yml
...
Ajoute à `exclude` dans `\_config.yml` : `package.json`, `package-lock.json`, `scripts`, `outils/cuisine/src`, `outils/cuisine/tests`, `outils/cuisine/README.md`, `outils/cuisine/DECISIONS.md`. Vérifie que les exclusions par défaut, dont `node\_modules`, restent actives.
...
\### 5.3 Catégories (rayons) — `\_data/cuisine/categories.json`
...
\### 5.4 Exemple complet — `\_data/cuisine/recettes/gratin-dauphinois.json`
...
permalink: /outils/cuisine/recettes.json

\---

{

&#x20; "genere": {{ site.time | date\_to\_xmlschema | jsonify }},

&#x20; "categories": {{ site.data.cuisine.categories | jsonify }},

&#x20; "recettes": \[{% for pair in site.data.cuisine.recettes %}{{ pair\[1] | jsonify }}{% unless forloop.last %},{% endunless %}{% endfor %}]
...
`outils/cuisine/ingredients.txt` contient une ligne `nom | categorie` par ingrédient distinct, triée. Le convertisseur Claude la lit pour réutiliser exactement les noms existants.
...
\### 5.6 Validation — `scripts/cuisine-check.mjs` (`npm run check`)
...
`.github/workflows/cuisine-check.yml` se déclenche sur les push et PR qui touchent `\_data/cuisine/\*\*`, `assets/cuisine/\*\*`, `outils/cuisine/\*\*` ou `scripts/\*\*`. Il utilise Node LTS et lance `npm ci`, `npm run check` et `npm test`. Il est indépendant du déploiement Pages : un échec signale le problème mais ne bloque pas la publication.
...
Une seule clé `localStorage` : `cuisine:v1`.
...
\- Le fichier `outils/cuisine/app.css` généré est commité.



```css

/\* outils/cuisine/src/app.css \*/
...
&#x20; - `name` « Cuisine — Le Hub de la Guite », `short\_name` « Cuisine » ;

&#x20; - `start\_url` et `scope` à `/outils/cuisine/`, `display: standalone` ;

&#x20; - icônes 192, 512 et maskable, couleurs du thème.

\- \*\*`sw.js`\*\* (avec front matter Liquid, `layout: null`) :

&#x20; - nom du cache `cuisine-{{ site.time | date: '%s' }}`, donc nouvelle version à chaque build, automatiquement ;

&#x20; - pré-cache de la coquille : `index.html`, `app.css`, `/assets/css/tokens.css`, `recettes.json`, et tous les `site.static\_files` dont le `path` commence par `/outils/cuisine/` (liste générée par Liquid, jamais maintenue à la main) ;
...
\- La page `pages/outils.md` existe déjà (`is\_main: true`, catégorie `tool`, dans la nav). Elle liste les articles de catégorie `tool`. \*\*Ne pas la modifier\*\*, ne pas créer de `pages/outils.html` ni de `\_data/outils.yml`.

\- Cuisine y apparaît automatiquement grâce à son article, comme icomkr.

\- Les apps existantes (`outils/icomkr.html`, `projets/flechettes.html`, `projets/yams.html`), leurs articles et la page Projets ne bougent pas.

\- \*\*Article `\_posts/AAAA-MM-JJ-cuisine.md`\*\* calqué sur celui d'icomkr :

&#x20; - lien vers `/outils/cuisine/` ;

&#x20; - sections « À quoi ça sert », « Comment on l'utilise », « Sur quelle techno c'est bâti » et « Ajouter une recette » ;

&#x20; - catégorie `tool`, vignette `assets/images/vignette-cuisine.\*`.
...
| \*\*5. Intégration au Hub\*\* | Article (catégorie `tool`), `exclude` | Cuisine apparaît sur la page Outils ; le reste du blog est identique à la référence de la phase 0 |

| \*\*6. Documentation\*\* | `README.md` (ajouter une recette en 4 étapes), `DECISIONS.md` à jour. En option, `npm run photos` (sharp : `\_photos/\*.jpg|png` → `assets/cuisine/recettes/<nom>.webp`, 1200 px de large, qualité 80, dossier `\_photos/` dans `.gitignore`) | Je peux ajouter une recette en suivant le README, sans aide |
...
\- \[ ] Ajouter `\_data/cuisine/recettes/x.json` et `assets/cuisine/recettes/x.webp`, puis pousser : la recette apparaît sans aucune modification de code.
